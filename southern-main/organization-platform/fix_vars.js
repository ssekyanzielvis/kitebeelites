...
files.forEach(filePath => {
  ...
  if (match && match[1]) {
    ...
    content = content.replace(/setPreviewMedia\([^)]*\)/g, `setPreviewMedia((${varName} as any).image_url || (${varName} as any).media_url)`);
    fs.writeFileSync(filePath, content);
    console.log('Fixed', filePath, 'using var:', varName);
  }
});
